...
import { loadRecognitionHints } from "./correction-memory.mjs";

export async function probeMedia(filePath) {
  const result = await runCommand("ffprobe", [
    "-v", "error",
    "-show_entries", "format=duration,size,format_name",
    "-show_entries", "stream=index,codec_type,codec_name,channels,sample_rate,width,height:stream_disposition=attached_pic",
    "-of", "json",
    filePath,
  ]);
  const payload = JSON.parse(result.stdout);
  const streams = payload.streams || [];
  // Podcast MP3/M4A files often embed a cover as a video stream; it is not footage.
  const video = streams.find((stream) => stream.codec_type === "video" && !Number(stream.disposition?.attached_pic)) || null;
...
export async function transcribeMedia(filePath, workDir, options = {}) {
  ...
  const recognitionHints = await loadRecognitionHints();
  options.onStage?.(`本地 Whisper 正在生成精确时间戳${recognitionHints.terms.length ? `（参考 ${recognitionHints.terms.length} 条纠错）` : ""}`, 0.52);
  await runCommand("whisper-cli", [
    "-m", modelPath,
    "-f", wavPath,
    "-l", "en",
    ...(recognitionHints.prompt ? ["--prompt", recognitionHints.prompt, "--carry-initial-prompt"] : []),
    "-ojf",
    "-sow",
    "-ng",
    "-nfa",
  ...
  return { blocks, jsonPath, recognitionHints: recognitionHints.terms };
}
