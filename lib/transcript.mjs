...
import { groupSentenceFragments, joinTimedSentenceParts, splitSentenceSpans } from "./sentence-boundaries.mjs";
...
export function blocksToSentences(blocks) {
  ...
  for (const block of blocks) {
    ...
    const parts = splitSentenceSpans(cleanedBlockText)
      .map(({ segment }) => segment.trim())
      .filter(Boolean);
    const segmentedParts = parts.length ? parts : [cleanedBlockText];
    const safeParts = segmentedParts;
  ...
  return groupSentenceFragments(sentences).map((parts, index) => ({
    ...joinTimedSentenceParts(parts), id: `sentence-${index + 1}`,
  }));
}
...
export function buildParagraphs(sentences, maxWords = 100, maxPauseSeconds = 2.5) {
  ...
  for (const sentence of sentences) {
    ...
    current.sourceBlockIds = [...new Set([...current.sourceBlockIds, sentence.sourceBlockId, ...(sentence.sourceBlockIds || [])].filter(Boolean))];
...
export function parseWhisperJson(payload, durationSeconds = null) {
  ...
  for (const entry of entries) {
    ...
    if (timedTokens.length) {
      ...
      const parts = splitSentenceSpans(combined);
