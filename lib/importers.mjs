...
import { downloadYouTubeMedia } from "./youtube.mjs";
import { downloadApplePodcast } from "./apple-podcasts.mjs";
import {
  blocksToSentences,
  buildParagraphs,
...
export async function importYouTubeMaterial(materialId, sourceUrl, report, options = {}) {
  const downloaded = await downloadYouTubeMedia(sourceUrl, materialDir(materialId), { onStage: report });
  await updateMaterial(materialId, (latest) => {
    latest.title = downloaded.title;
    latest.sourceUrl = downloaded.sourceUrl;
    latest.media = { file: downloaded.file };
    latest.stage = "YouTube 原声已保存，等待本地转写";
  });
  return importLocalMaterial(materialId, report, {
    ...options,
    warning: downloaded.audioOnly ? "此视频已导入原始音轨，可使用纯听模式。" : null,
  });
}

export async function importApplePodcastMaterial(materialId, sourceUrl, report, options = {}) {
  const downloaded = await downloadApplePodcast(sourceUrl, materialDir(materialId), { onStage: report });
  await updateMaterial(materialId, (latest) => {
    latest.title = downloaded.title;
    latest.sourceUrl = downloaded.sourceUrl;
    latest.media = { file: downloaded.file };
    latest.stage = "播客原声已保存，等待本地转写";
  });
  return importLocalMaterial(materialId, report, options);
}
...
export async function importLocalMaterial(materialId, report, options = {}) {
  ...
  material = await updateMaterial(materialId, (latest) => {
    ...
    latest.status = "ready";
    latest.stage = "可以开始精听，讲解正在生成";
    latest.warning = options.warning || null;
    return latest;
  });
  await addAnalysis(materialId, report, options);
  return await readMaterial(materialId);
