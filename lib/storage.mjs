...
import { extractCorrectionHints } from "../public/transcript-correction-utils.js";
import { canonicalParagraphId, canonicalSentenceId, normalizeMaterialSentenceBoundaries } from "./material-sentence-boundaries.mjs";
...
export async function purgeExpiredTrash(options = {}) {
  if (process.env.LISTENING_PRESERVE_DATA === "1") return [];
...
export async function updateSentenceText(id, sentenceId, text, options = {}) {
  return updateMaterial(id, (material) => {
    ...
    const correction = {
      id: createId("correction"), sentenceId,
      previousText: sentence.text, correctedText: nextText,
      enabled: options.learnForRecognition !== false && hints.length > 0,
      createdAt: new Date().toISOString(),
    };
    material.transcriptCorrections = [...(material.transcriptCorrections || []).map(item => (
      item.sentenceId === sentenceId ? { ...item, enabled: false } : item
    )), correction];
    sentence.text = nextText;
    sentence.wordCount = countWords(nextText);
    sentence.analysis = null;
    delete sentence.previousAnalysis;
    ...
    return { material, sentence, changed: true, correction, recognitionHints: hints };
  });
}
...
export async function savePhraseGuideItem(id, guide) {
  return updateMaterial(id, (material) => {
    ...
    const analyzedPhrase = ((sentence.analysis || sentence.previousAnalysis)?.phrases || []).find((item) => (
...
export async function saveReviewItem(id, reviewItem) {
  return updateMaterial(id, (material) => {
    ...
    if (reviewItem.sentenceId) reviewItem.sentenceId = canonicalSentenceId(material, reviewItem.sentenceId);
    if (reviewItem.paragraphId) reviewItem.paragraphId = canonicalParagraphId(material, reviewItem.paragraphId);
    const now = new Date().toISOString();
    const item = {
      ...reviewItem,
...
export async function saveQaHistoryItem(id, historyItem) {
  return updateMaterial(id, (material) => {
    historyItem = { ...historyItem, sentenceId: canonicalSentenceId(material, historyItem.sentenceId) };
    const now = new Date().toISOString();
    const item = {
      ...historyItem,
